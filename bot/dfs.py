...
def findBestTradeExactIn(
    ...
    for i in range(len(pairs)):
        pair = pairs[i]
        if (
            not pair["token0"]["address"] == tokenIn["address"]
            and not pair["token1"]["address"] == tokenIn["address"]
        ):
            continue
...
def findArb(pairs, tokenIn, tokenOut, maxHops, currentPairs, path, bestTrades, count=5):
    for i in range(len(pairs)):
        ...
        if (
            ...
            continue
        if (
            pair["reserve0"] / pow(10, pair["token0"]["decimal"]) < 1
            or pair["reserve1"] / pow(10, pair["token1"]["decimal"]) < 1
        ):
            continue
        if tokenIn["address"] == pair["token0"]["address"]:
            tempOut = pair["token1"]
        ...
        if tempOut["address"] == tokenOut["address"] and len(path) > 2:
            ...
            if Ea and Eb and Ea < Eb:
                ...
                if newTrade["optimalAmount"] > 0:
                    ...
                    newTrade["profit"] = (
                        newTrade["outputAmount"] - newTrade["optimalAmount"]
                    )
                    newTrade["p"] = int(newTrade["profit"]) / pow(
                        10, tokenOut["decimal"]
                    )
                else:
                    continue
