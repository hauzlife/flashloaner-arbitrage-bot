    ...
    for i in range(len(pairs)):
        ...
        if tokenIn['address'] == pair['token0']['address']:
            amountOut = getAmountOut(
                amountIn, pair['reserve0'], pair['reserve1'])
            tempOut = pair['token0']
        if tokenIn['address'] == pair['token1']['address']:
            amountOut = getAmountOut(
                amountIn, pair['reserve1'], pair['reserve0'])
            tempOut = pair['token1']
        path.append(tempOut)
        if tempOut == tokenOut:
            newTrade = {'route': currentPairs + [pair], 'path': path, 'tokenIn': origToken,
                        'tokenOut': tokenOut, 'amountIn': origAmount, 'amountOut': amountOut}
            if not bestTrade:
                bestTrade = newTrade
            ...
            findBestTradeExactIn(pairsExcludingThisPair, tempOut, tokenOut, amountOut,
                                 maxHops-1, currentPairs + [pair], path, origToken, origAmount, bestTrade)
        return bestTrade
...
def findArb(pairs, tokenIn, tokenOut, maxHops, currentPairs, path, bestTrades, count=5):
    for i in range(len(pairs)):
        ...
        if tempOut['address'] == tokenOut['address'] and len(path) > 2:
            ...
            newTrade = {'route': currentPairs +
                        [pair], 'path': newPath, 'Ea': Ea, 'Eb': Eb}
            if Ea and Eb and Ea < Eb:
                newTrade['optimalAmount'] = getOptimalAmount(Ea, Eb)
                if newTrade['optimalAmount'] > 0:
                    newTrade['outputAmount'] = getAmountOut(
                        newTrade['optimalAmount'], Ea, Eb)
                    newTrade['profit'] = newTrade['outputAmount'] - \
                        newTrade['optimalAmount']
                    newTrade['p'] = int(newTrade['profit']) / \
                        pow(10, tokenOut['decimal'])
                else:
                    continue
