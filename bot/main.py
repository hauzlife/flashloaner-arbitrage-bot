...
def printMoney(amountIn, p, gasPrice, profit):
    ...
    tx = printer.functions.printMoney(
        startToken["address"], amountIn, amountIn, p, deadline
    ).buildTransaction(
        {
            "from": address,
            "value": 0,
    ...
    if (
        config["start"] == "usdt"
        or config["start"] == "usdc"
        or config["start"] == "dai"
    ):
        if gasEstimate * gasPrice / 1e18 * 360 >= profit / pow(
            10, startToken["decimal"]
        ):
...
def flashPrintMoney(amountIn, p, gasPrice, profit):
    tx = printer.functions.flashPrintMoney(
        startToken["address"], amountIn, p
    ).buildTransaction(
        {
            "from": address,
            "value": 0,
