...
def get_reserves(pairs, blockNumber="latest"):
    ...
    for i in range(len(results)):
        res = decode_abi(
            ["uint256", "uint256", "uint256"], bytes.fromhex(results[i][2:])
        )
    ...
    for r in receipts:
        ...
        for log in r["logs"]:
            ...
            if log["topics"][0] == sync_topic:
                ...
                pairs[pairsDict[log["address"]]["arrIndex"]]["reserve0"] = event[
                    "reserve0"
                ]
                pairs[pairsDict[log["address"]]["arrIndex"]]["reserve1"] = event[
                    "reserve1"
                ]
    return pairs
